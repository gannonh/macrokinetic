...
struct DoseCalendarViewModelTests {
    ...
    func setDosesUpdatesCalendarData() throws {
        ...
        let doses = [
            ...
            createTestDose(
                timestamp: calendar.date(byAdding: .day, value: 5, to: monthStart) ?? monthStart,
                amount: 1.5),
            self.createTestDose(
                timestamp: calendar.date(byAdding: .month, value: -1, to: monthStart) ?? monthStart,
        ...
        #expect(
            self.viewModel.monthlyDoses.count == 2, "Only current month doses should be in monthly doses")
    ...
    func testNavigateToPreviousMonth() throws {
        ...
        #expect(
            calendar.isDate(self.viewModel.currentMonth, equalTo: expectedMonth, toGranularity: .month),
    ...
    func testNavigateToCurrentMonth() throws {
        ...
        #expect(
            calendar.isDate(self.viewModel.currentMonth, equalTo: today, toGranularity: .month),
    ...
    func navigateToSpecificMonth() throws {
        ...
        #expect(
            calendar.isDate(self.viewModel.currentMonth, equalTo: targetMonth, toGranularity: .month),
    ...
    func monthStartAndEndCalculation() throws {
        ...
        #expect(
            calendar.isDate(monthStart, equalTo: expectedStart, toGranularity: .day),
            "Month start should be first day of month")
        #expect(
            calendar.isDate(monthEnd, equalTo: expectedEnd, toGranularity: .day),
    ...
    func daysInMonthCalculation() throws {
        ...
        for testCase in testCases {
            let testDate = Calendar.current.date(
                from: DateComponents(year: testCase.year, month: testCase.month, day: 1))!
            self.viewModel.currentMonth = testDate
            ...
            #expect(
                daysInMonth.count == testCase.expectedDays,
    ...
    func calculateStatisticsForMonth() throws {
        ...
        let doses = [
            ...
            createTestDose(
                timestamp: calendar.date(byAdding: .day, value: 7, to: monthStart) ?? monthStart,
                amount: 1.5, skipped: false),
            self.createTestDose(
                timestamp: calendar.date(byAdding: .day, value: 14, to: monthStart) ?? monthStart,
        ...
        #expect(
            abs(stats.averageDose - 1.25) < 0.001, "Average should be calculated from non-skipped doses")
    }
