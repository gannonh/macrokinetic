...
struct DoseHistoryViewModelComputedTests {
    ...
    func availableInjectionSitesForFiltering() async throws {
        ...
        let dose3 = self.createTestDose(timestamp: Date().addingTimeInterval(7200), amount: 1.5, site: "Abdomen")
    ...
    func dosesForSpecificDate() async throws {
        ...
        let calendar = Calendar.current
        let targetDate = Date()
        let morningTime = try #require(calendar.date(bySettingHour: 8, minute: 0, second: 0, of: targetDate))
        let eveningTime = try #require(calendar.date(bySettingHour: 20, minute: 30, second: 0, of: targetDate))
