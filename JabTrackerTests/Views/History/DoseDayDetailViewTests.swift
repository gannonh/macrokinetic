...
struct DoseDayDetailViewTests {
  ...
  func doseDayDetailViewDisplaysMultipleDosesChronologically() throws {
    ...
    guard let afternoonTime = calendar.date(bySettingHour: 14, minute: 0, second: 0, of: date)
    else {
  ...
  func doseDayDetailViewFormatsDoseTimes() throws {
    ...
    let calendar = Calendar.current
    let date = Date()
    guard let specificTime = calendar.date(bySettingHour: 14, minute: 30, second: 0, of: date)
    else {
