...
struct DoseHistoryRowTests {
    ...
    func testBasicDoseRowCreation() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        let medication = createTestMedicationProfile(context: context, user: user)
    ...
    func testDoseRowWithNoMedication() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        let dose = createTestDose(context: context, user: user, medication: nil)
    ...
    func testDoseRowWithNoInjectionSite() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        let medication = createTestMedicationProfile(context: context, user: user)
        let dose = createTestDose(context: context, user: user, medication: medication, site: nil)
    ...
    func testDoseRowWithNoNotes() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        let medication = createTestMedicationProfile(context: context, user: user)
        let dose = createTestDose(context: context, user: user, medication: medication, notes: nil)
    ...
    func testSkippedDoseRow() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        let medication = createTestMedicationProfile(context: context, user: user)
        let dose = createTestDose(context: context, user: user, medication: medication, skipped: true)
    ...
    func testDoseRowWithPhoto() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        let medication = createTestMedicationProfile(context: context, user: user)
        let dose = createTestDose(context: context, user: user, medication: medication, hasPhoto: true)
    ...
    func testAccessibilityLabelIncludesDoseAmount() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        let medication = createTestMedicationProfile(context: context, user: user)
        let dose = createTestDose(context: context, user: user, medication: medication, amount: 2.5)
    ...
    func testAccessibilityLabelIncludesMedication() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        let medication = createTestMedicationProfile(context: context, user: user)
        let dose = createTestDose(context: context, user: user, medication: medication)
    ...
    func testAccessibilityLabelIncludesSkippedStatus() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        let medication = createTestMedicationProfile(context: context, user: user)
        let dose = createTestDose(context: context, user: user, medication: medication, skipped: true)
    ...
    func testAccessibilityLabelIncludesInjectionSite() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        let medication = createTestMedicationProfile(context: context, user: user)
        let dose = createTestDose(context: context, user: user, medication: medication, site: "Left thigh")
    ...
    func testAccessibilityLabelIncludesPhotoStatus() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        let medication = createTestMedicationProfile(context: context, user: user)
        let dose = createTestDose(context: context, user: user, medication: medication, hasPhoto: true)
    ...
    func testAccessibilityLabelIncludesNotesStatus() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        let medication = createTestMedicationProfile(context: context, user: user)
        let dose = createTestDose(context: context, user: user, medication: medication, notes: "Important note")
    ...
    func testDoseAmountFormatting() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        let medication = createTestMedicationProfile(context: context, user: user)
        ...
        for amount in testAmounts {
            let dose = createTestDose(context: context, user: user, medication: medication, amount: amount)
            let row = DoseHistoryRow(dose: dose)
    ...
    func testLongNotesHandling() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        let medication = createTestMedicationProfile(context: context, user: user)
        
        let longNotes = String(repeating: "This is a very long note that should be truncated. ", count: 10)
        let dose = createTestDose(context: context, user: user, medication: medication, notes: longNotes)
    ...
    func testEmptyNotesHandling() throws {
        ...
        let dose = createTestDose(context: context, user: user, medication: medication, notes: "")
        let row = DoseHistoryRow(dose: dose)
    ...
    func testMedicationDisplayPriority() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        ...
        let dose1 = createTestDose(context: context, user: user, medication: medicationWithBoth)
        ...
        let dose2 = createTestDose(context: context, user: user, medication: medicationGenericOnly)
    ...
    func testNilInjectionSiteDisplay() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        let medication = createTestMedicationProfile(context: context, user: user)
    ...
    func testEmptyInjectionSiteDisplay() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        let medication = createTestMedicationProfile(context: context, user: user)
        
        let dose = createTestDose(context: context, user: user, medication: medication, site: "")
        let row = DoseHistoryRow(dose: dose)
    ...
    func testStatusIndicatorSkippedState() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        let medication = createTestMedicationProfile(context: context, user: user)
        
        // Test normal dose
        let normalDose = createTestDose(context: context, user: user, medication: medication, skipped: false)
        let normalRow = DoseHistoryRow(dose: normalDose)
        
        #expect(normalRow.dose.skipped == false)
        
        // Test skipped dose
        let skippedDose = createTestDose(context: context, user: user, medication: medication, skipped: true)
    ...
    func testPhotoIndicatorVisibility() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        let medication = createTestMedicationProfile(context: context, user: user)
        
        // Test dose without photo
        let doseNoPhoto = createTestDose(context: context, user: user, medication: medication, hasPhoto: false)
        let rowNoPhoto = DoseHistoryRow(dose: doseNoPhoto)
        
        #expect(rowNoPhoto.dose.imageData == nil)
        
        // Test dose with photo
        let doseWithPhoto = createTestDose(context: context, user: user, medication: medication, hasPhoto: true)
    ...
    func testTimestampFormatting() throws {
        ...
        let specificTime = Date(timeIntervalSince1970: 1640995200) // Known timestamp
        let dose = createTestDose(context: context, user: user, medication: medication, timestamp: specificTime)
    ...
    func testContentShapeConfiguration() throws {
        let context = createTestModelContext()
        let user = createTestUser(context: context)
        let medication = createTestMedicationProfile(context: context, user: user)
        let dose = createTestDose(context: context, user: user, medication: medication)
...
struct DoseHistoryRowEdgeCaseTests {
    ...
    func testExtremeDoseAmounts() throws {
        let context = createTestModelContext()
        let user = User(email: "test@example.com", name: "Test User")
        context.insert(user)
        try! context.save()
    ...
    func testVeryOldTimestamps() throws {
        let context = createTestModelContext()
        let user = User(email: "test@example.com", name: "Test User")
        context.insert(user)
        try! context.save()
