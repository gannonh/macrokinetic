...
struct DoseHistoryRowTests {
    ...
    func skippedDoseRow() throws {
        ...
        let user = self.createTestUser(context: context)
        let medication = self.createTestMedicationProfile(context: context, user: user)
        let dose = self.createTestDose(context: context, user: user, medication: medication, skipped: true)
    ...
    func doseRowWithPhoto() throws {
        ...
        let user = self.createTestUser(context: context)
        let medication = self.createTestMedicationProfile(context: context, user: user)
        let dose = self.createTestDose(context: context, user: user, medication: medication, hasPhoto: true)
    ...
    func accessibilityLabelIncludesDoseAmount() throws {
        ...
        let user = self.createTestUser(context: context)
        let medication = self.createTestMedicationProfile(context: context, user: user)
        let dose = self.createTestDose(context: context, user: user, medication: medication, amount: 2.5)
    ...
    func accessibilityLabelIncludesInjectionSite() throws {
        ...
        let user = self.createTestUser(context: context)
        let medication = self.createTestMedicationProfile(context: context, user: user)
        let dose = self.createTestDose(context: context, user: user, medication: medication, site: "Left thigh")
    ...
    func accessibilityLabelIncludesNotesStatus() throws {
        ...
        let user = self.createTestUser(context: context)
        let medication = self.createTestMedicationProfile(context: context, user: user)
        let dose = self.createTestDose(context: context, user: user, medication: medication, notes: "Important note")
    ...
    func doseAmountFormatting() throws {
        ...
        for amount in testAmounts {
            let dose = self.createTestDose(context: context, user: user, medication: medication, amount: amount)
            let row = DoseHistoryRow(dose: dose)
    ...
    func longNotesHandling() throws {
        ...
        let dose = self.createTestDose(context: context, user: user, medication: medication, notes: longNotes)
    ...
    func statusIndicatorSkippedState() throws {
        ...
        // Test normal dose
        let normalDose = self.createTestDose(context: context, user: user, medication: medication, skipped: false)
        let normalRow = DoseHistoryRow(dose: normalDose)

        #expect(normalRow.dose.skipped == false)

        // Test skipped dose
        let skippedDose = self.createTestDose(context: context, user: user, medication: medication, skipped: true)
    ...
    func photoIndicatorVisibility() throws {
        ...
        let doseNoPhoto = self.createTestDose(context: context, user: user, medication: medication, hasPhoto: false)
        let rowNoPhoto = DoseHistoryRow(dose: doseNoPhoto)

        #expect(rowNoPhoto.dose.imageData == nil)

        // Test dose with photo
        let doseWithPhoto = self.createTestDose(context: context, user: user, medication: medication, hasPhoto: true)
    ...
    func timestampFormatting() throws {
        ...
        let dose = self.createTestDose(context: context, user: user, medication: medication, timestamp: specificTime)
