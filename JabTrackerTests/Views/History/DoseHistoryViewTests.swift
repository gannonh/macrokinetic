...
struct DoseHistoryViewTests {
    ...
    private func createTestMedicationProfile(context: ModelContext, user: User) throws
        -> MedicationProfile
    {
    ...
    func viewModelLoadData() async throws {
        ...
        let dose1 = try self.createTestDose(
            context: context, user: user, medication: medication, amount: 1.0)
    ...
    func viewModelFilters() async throws {
        ...
        _ = try self.createTestDose(
            context: context, user: user, medication: medication1, site: "Thigh")
        _ = try self.createTestDose(
            context: context, user: user, medication: medication2, site: "Abdomen")
        _ = try self.createTestDose(
            context: context,
            user: user,
    ...
    func viewModelCRUD() async throws {
        ...
        let (
            duplicatedDoseExists, duplicatedAmount, duplicatedSite, duplicatedNotes, duplicatedSkipped
        ) =
            await MainActor.run {
    ...
    func viewModelGroupedDoses() async throws {
        ...
        _ = try self.createTestDose(
            context: context, user: user, medication: medication, timestamp: today)
        _ = try self.createTestDose(
            context: context,
            user: user,
            medication: medication,
        ...
        _ = try self.createTestDose(
            context: context, user: user, medication: medication, timestamp: yesterday)
        _ = try self.createTestDose(
            context: context, user: user, medication: medication, timestamp: twoDaysAgo)
    ...
    func viewModelAvailableData() async throws {
        ...
        _ = try self.createTestDose(
            context: context, user: user, medication: medication2, site: "Abdomen")
        _ = try self.createTestDose(context: context, user: user, medication: medication1, site: "Arm")
    ...
    func viewModelRefresh() async throws {
        ...
        _ = try self.createTestDose(context: context, user: user, medication: medication)
