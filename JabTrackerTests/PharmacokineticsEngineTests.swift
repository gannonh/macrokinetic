...
struct PharmacokineticsEngineTests {
    ...
    func createTestDoses(
        amounts: [Double],
        daysAgo: [Double],
        medicationProfile: MedicationProfile) -> [Dose]
    {
    ...
    func singleDoseConcentrationSemaglutide() throws {
        let medicationProfile = self.createTestMedicationProfile(medication: .semaglutide, currentDose: 1.0)
        let dose = Dose(
            amount: 1.0,
    ...
    func singleDoseConcentrationTirzepatide() throws {
        let medicationProfile = self.createTestMedicationProfile(medication: .tirzepatide, currentDose: 5.0)
        let dose = Dose(
            amount: 5.0,
            timestamp: testDate.addingTimeInterval(-24 * 3600), // 1 day ago
            medication: medicationProfile)
    ...
    func singleDoseConcentrationLiraglutide() throws {
        let medicationProfile = self.createTestMedicationProfile(medication: .liraglutide, currentDose: 1.8)
        let dose = Dose(
            amount: 1.8,
    ...
    func escalatingDosePattern() throws {
        let medicationProfile = self.createTestMedicationProfile(medication: .semaglutide, currentDose: 2.0)
        let doses = self.createTestDoses(
    ...
    func calculateSteadyStateProgressDifferentMedications() throws {
        ...
        for medication in medications {
            ...
            medicationProfile.startDate = self.testDate.addingTimeInterval(-medication.halfLifeDays * 24 * 3600)
    ...
    func handleLargeDoseAmounts() throws {
        let medicationProfile = self.createTestMedicationProfile(medication: .semaglutide, currentDose: 100.0)
        let largeDose = Dose(
            amount: 100.0,
            timestamp: testDate,
            medication: medicationProfile)
    ...
    func handleSmallDoseAmounts() throws {
        let medicationProfile = self.createTestMedicationProfile(medication: .semaglutide, currentDose: 0.001)
        let smallDose = Dose(
            amount: 0.001,
            timestamp: testDate,
            medication: medicationProfile)
    ...
    func validateHalfLifeDecayAccuracy() throws {
        ...
        for medication in medications {
            let medicationProfile = self.createTestMedicationProfile(medication: medication, currentDose: 1.0)
            let dose = Dose(
                amount: 1.0,
    ...
    func validateSteadyStateTiming() throws {
        ...
        for medication in medications {
            ...
            medicationProfile.startDate = self.testDate.addingTimeInterval(-5 * medication.halfLifeDays * 24 * 3600)
