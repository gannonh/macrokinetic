...
struct AdherenceStatisticsTests {
    ...
    func adherenceRateCalculation() throws {
        ...
        #expect(
            adherencePercentage == "66.7%", "Adherence percentage string should be formatted correctly")
    }
    ...
    func calculatorBasicCounts() throws {
        ...
        let doses = [
            ...
            createTestDose(
                timestamp: Calendar.current.date(byAdding: .day, value: 1, to: startDate) ?? startDate,
                amount: 1.5, site: "Abdomen", skipped: false),
            self.createTestDose(
                timestamp: Calendar.current.date(byAdding: .day, value: 2, to: startDate) ?? startDate,
                amount: 1.2, site: "Thigh", skipped: true),
            self.createTestDose(
                timestamp: Calendar.current.date(byAdding: .day, value: 10, to: startDate) ?? startDate,
        ...
        #expect(
            stats.scheduledDoses == 1,
            "Should calculate 1 scheduled dose for weekly frequency over 7 days")
    ...
    func calculatorSiteDistribution() throws {
        ...
        let doses = [
            ...
            self.createTestDose(
                timestamp: Calendar.current.date(byAdding: .day, value: 3, to: startDate) ?? startDate,
                site: "Thigh", skipped: true),  // Skipped, but still included in site distribution logic
            self.createTestDose(
                timestamp: Calendar.current.date(byAdding: .day, value: 4, to: startDate) ?? startDate,
        ...
        #expect(
            stats.siteDistribution == expectedSites,
            "Site distribution should count non-skipped doses with sites")
    ...
    func calculatorWeeklyScheduledDoses() throws {
        ...
        for testCase in testCases {
            let endDate =
                Calendar.current.date(byAdding: .day, value: testCase.days, to: startDate) ?? startDate
            ...
            #expect(
                stats.scheduledDoses == testCase.expected,
    ...
    func calculatorStreakCalculation() throws {
        ...
        #expect(
            stats.isCurrentStreakActive == false, "Current streak should be inactive (no dose today)")
    }
