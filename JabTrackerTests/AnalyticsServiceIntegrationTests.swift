...
struct AnalyticsServiceIntegrationTests {
    ...
    @MainActor func userAnalyticsIntegration() throws {
        ...
        let dose2 = self.createTestDose(
            amount: 1.0, medicationProfile: semaglutideProfile, user: user, daysAgo: 14)
        let dose3 = self.createTestDose(
            amount: 2.5, medicationProfile: tirezepatideProfile, user: user, daysAgo: 10)
        ...
        #expect(
            userAnalytics.concentrationTrends.count == 2, "Should generate trends for both medications")
        #expect(
            userAnalytics.overallAdherence >= 0.0 && userAnalytics.overallAdherence <= 1.0,
            "Adherence should be normalized")
    ...
    @MainActor func crossMedicationAdherence() throws {
        ...
        for week in 0..<4 {
            let dose = self.createTestDose(
                amount: 1.0, medicationProfile: semaglutideProfile, user: user, daysAgo: week * 7)
            dose.user = user
        ...
        for week in [0, 2] {  // Skip weeks 1 and 3
            let dose = self.createTestDose(
                amount: 2.5, medicationProfile: tirezepatideProfile, user: user, daysAgo: week * 7)
            dose.user = user
        ...
        #expect(
            overallAdherence >= 0.9, "Overall adherence should be high when all recorded doses are taken")
    }
    ...
    @MainActor func concentrationAdherenceCorrelation() throws {
        ...
        let dose3 = self.createTestDose(
            amount: 1.0, medicationProfile: profile, user: user, daysAgo: 21)
        ...
        let analyticsService = AnalyticsService()
        let concentrationInsights = analyticsService.generateConcentrationInsights(
        ...
        #expect(
            semaglutideInsight!.currentConcentration >= 0.0, "Should calculate current concentration")
        #expect(
            semaglutideInsight!.adherenceImpact >= 0.0 && semaglutideInsight!.adherenceImpact <= 1.0,
            "Adherence impact should be normalized")
    ...
    @MainActor func medicationEffectivenessScoring() throws {
        ...
        for week in 0..<4 {  // 4 weeks = 28 days, all within 30-day window
            let dose = self.createTestDose(
                amount: 1.0, medicationProfile: profile, user: user, daysAgo: week * 7)
            dose.user = user
        ...
        let analyticsService = AnalyticsService()
        let effectiveness = analyticsService.calculateMedicationEffectiveness(
            profile: profile, user: user, context: context)

        #expect(effectiveness.medicationProfile == profile, "Should reference correct profile")
        #expect(
            effectiveness.effectivenessScore >= 0.0 && effectiveness.effectivenessScore <= 1.0,
            "Score should be normalized")
        #expect(
            effectiveness.adherenceRate >= 0.0 && effectiveness.adherenceRate <= 1.0,
            "Adherence rate should be normalized")
        #expect(
    ...
    @MainActor func insightGeneration() throws {
        ...
        let excellentAdherenceInsight = userAnalytics.adherenceInsights.first {
            $0.type == .excellentAdherence
        }
        ...
        #expect(
            !excellentAdherenceInsight!.actionableRecommendation.isEmpty, "Should provide recommendation")
    }
    ...
    @MainActor func edgeCaseHandling() throws {
        ...
        let emptyProfile = self.createTestMedicationProfile(
            medication: .semaglutide, user: userWithoutDoses)
        ...
        let analyticsWithoutMeds = analyticsService.calculateUserAnalytics(
            user: userWithoutMedications, context: context)
        ...
        let analyticsWithoutDoses = analyticsService.calculateUserAnalytics(
            user: userWithoutDoses, context: context)
        #expect(analyticsWithoutDoses.medicationEffectiveness.count == 1, "Should include medication")
        #expect(
            analyticsWithoutDoses.medicationEffectiveness.first?.effectivenessScore == 0.0,
            "Should show zero effectiveness")
    }
    ...
    @MainActor func dataConsistency() throws {
        ...
        // Verify consistency
        #expect(
            analytics1.overallAdherence == analytics2.overallAdherence,
            "Adherence calculations should be consistent")
        #expect(
            analytics1.totalActiveDays == analytics2.totalActiveDays, "Active days should be consistent")
        #expect(
            analytics1.medicationEffectiveness.count == analytics2.medicationEffectiveness.count,
            "Effectiveness count should be consistent")
        ...
        let profileAdherence = profile.calculateAdherence(
            from: thirtyDaysAgo, to: Date(), context: context)
        #expect(
            abs(analytics1.overallAdherence - profileAdherence) < 0.001,
            "Overall adherence should match profile adherence for single medication")
    }
    ...
    @MainActor func individualModelFunctionalityPreservation() throws {
        ...
        let newProfileAdherence = profile.calculateAdherence(
            ...
            context: context)
        #expect(
            abs(newProfileAdherence - profileAdherence) < 0.001,
            "Profile calculations should be preserved")
