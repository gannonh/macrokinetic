...
struct MedicationTests {
  ...
  func dosingFrequency() throws {
    ...
    for medication in Medication.allCases {
      ...
      #expect(
        validFrequencies.contains(freq),
  ...
  func availableDoses() throws {
    ...
    for medication in Medication.allCases {
      ...
      #expect(
        doses == doses.sorted(), "Doses should be in ascending order for \(medication.rawValue)")
    }
  }
  ...
  func brandNames() throws {
    ...
    for medication in Medication.allCases {
      ...
      #expect(
        medication.brands.allSatisfy { !$0.isEmpty },
  ...
  func medicationDescriptions() throws {
    ...
    #expect(
      semaDescription.contains("weekly"), "Semaglutide description should mention weekly dosing")

    #expect(
      tirzeDescription.contains("GIP/GLP-1"),
      "Tirzepatide description should mention dual mechanism")
    #expect(
      tirzeDescription.contains("weekly"), "Tirzepatide description should mention weekly dosing")

    #expect(liraDescription.contains("GLP-1"), "Liraglutide description should mention GLP-1")
    #expect(
      liraDescription.contains("daily"), "Liraglutide description should mention daily dosing")

    #expect(dulaDescription.contains("GLP-1"), "Dulaglutide description should mention GLP-1")
    #expect(
      dulaDescription.contains("weekly"), "Dulaglutide description should mention weekly dosing")
    ...
    for medication in Medication.allCases {
      ...
      #expect(
        description.count > 20, "Description should be substantial for \(medication.rawValue)")
    }
  }
  ...
  func colorHexCodes() throws {
    ...
    for medication in Medication.allCases {
      ...
      #expect(
        !seenColors.contains(colorHex), "Color hex should be unique for \(medication.rawValue)")
  ...
  func rawValueConsistency() throws {
    ...
    for medication in Medication.allCases {
      ...
      #expect(
        recreated == medication, "Raw value round-trip should work for \(medication.rawValue)")
    }
  }
  ...
  func pharmacokineticConsistency() throws {
    ...
    for medication in weeklyMedications {
      #expect(
        liraHalfLife < medication.halfLifeDays,
