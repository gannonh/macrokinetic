...
struct AuthenticationDelegateMethodTests {
    ...
    func authDelegateErrorHandlingIntegration() throws {
        ...
        let mockError = NSError(
            domain: "test.error", code: -1, userInfo: [NSLocalizedDescriptionKey: "Test error"])
    ...
    func authPresentationAnchorDelegateMethod() throws {
        ...
        #expect(
            window.windowLevel >= .normal, "Window should have appropriate level for authentication UI")
        ...
        Task {
            ...
            #expect(
                window.windowLevel >= .normal, "Window should be properly configured for authentication UI")
        }
    }
