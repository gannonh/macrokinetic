...
struct UserModelTests {
    ...
    func userEmailForCloudKitProperty() throws {
        ...
        #expect(userWithEmail.emailForCloudKit == "valid@example.com", "Should return actual email when available")

        // Test with nil email
        let userNoEmail = User(email: nil)
        context.insert(userNoEmail)

        #expect(userNoEmail.emailForCloudKit == "", "Should return empty string for nil email (CloudKit compatibility)")
    ...
    func userDisplayEmailProperty() throws {
        ...
        #expect(userWithEmail.displayEmail == "display@example.com", "Should return actual email for display")

        // Test with nil email
        let userNoEmail = User(email: nil)
        context.insert(userNoEmail)

        #expect(userNoEmail.displayEmail == "No email", "Should return 'No email' for nil email display")
    ...
    func userModelAppleUserIdLinking() throws {
        ...
        // Should persist
        #expect(user.appleUserId == "test.apple.user.id.12345", "Apple User ID should persist after save")
    }
