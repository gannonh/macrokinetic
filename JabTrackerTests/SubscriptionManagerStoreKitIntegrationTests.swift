...
@MainActor
@Suite("SubscriptionManager StoreKit Tests", .serialized)
struct SubscriptionManagerStoreKitTests {
    ...
    func loadProductsSuccessPath() async throws {
        ...
        let productIds = manager.availableProducts.map(\.id)
        ...
        if manager.availableProducts.count > 1 {
            for index in 1..<manager.availableProducts.count {
                #expect(
                    manager.availableProducts[index - 1].price <= manager.availableProducts[index].price)
            }
        }
    }
    ...
    func evaluateStatusComprehensive() async {
        ...
        let nonRenewable = SubscriptionManager.EvalInputTest(
            ...
            purchaseDate: now,
            expirationDate: nil)
        let nonRenewableStatus = SubscriptionManager.evaluateStatusForTests(
            from: [nonRenewable], now: now)
    ...
    func productFiltering() async throws {
        ...
        // Verify no overlap
        let monthlyIds = Set(monthly.map(\.id))
        let annualIds = Set(annual.map(\.id))
