...
struct DoseDefaultsTests {
  ...
  func nextRecommendedSiteWithRotation() throws {
    ...
    let dose1 = Dose(amount: 1.0, timestamp: Date().addingTimeInterval(-604_800), site: "Thigh")  // 1 week ago
  ...
  func nextScheduledDoseWeeklyMedication() throws {
    let profile = self.createTestMedicationProfile(
      ...
      startDate: Date().addingTimeInterval(-604_800)  // 1 week ago
    )
  ...
  func nextScheduledDoseNoPreviousDoses() throws {
    let startDate = Date().addingTimeInterval(-172_800)  // 2 days ago
    let profile = self.createTestMedicationProfile(
      medication: .semaglutide,
      startDate: startDate)
  ...
  func doseOverdueWeeklyMedication() throws {
    let profile = self.createTestMedicationProfile(
      medication: .semaglutide,
      startDate: Date().addingTimeInterval(-604_800 * 2)  // 2 weeks ago
    )

    // Last dose was over a week ago (should be overdue)
    let lastDose = Dose(
      amount: 1.0,
      timestamp: Date().addingTimeInterval(-604_800 - 10800)  // 1 week + 3 hours ago
    )
  ...
  func doseNotOverdueWithinGracePeriod() throws {
    ...
    let lastDose = Dose(
      amount: 1.0,
      timestamp: Date().addingTimeInterval(-604_800)  // Exactly 1 week ago
    )
  ...
  func doseOverdueDailyMedication() throws {
    let profile = self.createTestMedicationProfile(
      medication: .liraglutide,
      startDate: Date().addingTimeInterval(-172_800)  // 2 days ago
    )
  ...
  func adherenceCalculationPerfect() throws {
    let profile = self.createTestMedicationProfile(
      medication: .semaglutide,
      startDate: Date().addingTimeInterval(-1_209_600)  // 2 weeks ago
    )

    // Create doses for perfect adherence (2 weekly doses)
    let dose1 = Dose(amount: 1.0, timestamp: Date().addingTimeInterval(-604_800))  // 1 week ago
    let dose2 = Dose(amount: 1.0, timestamp: Date().addingTimeInterval(-86400))  // 1 day ago

    let timeRange = DateInterval(
      start: Date().addingTimeInterval(-1_209_600),  // 2 weeks ago
      end: Date())

    let adherence = DoseDefaults.calculateAdherence(
      for: profile, in: timeRange, doses: [dose1, dose2])
  ...
  func adherenceCalculationPartial() throws {
    ...
    let dose1 = Dose(amount: 1.0, timestamp: Date().addingTimeInterval(-1_209_600))  // 2 weeks ago
    let dose2 = Dose(amount: 1.0, timestamp: Date().addingTimeInterval(-86400))  // 1 day ago
    // Missing dose from 1 week ago

    let timeRange = DateInterval(
      start: Date().addingTimeInterval(-1_814_400),  // 3 weeks ago
      end: Date())
  ...
  func adherenceCalculationWithSkippedDoses() throws {
    ...
    let dose1 = Dose(amount: 1.2, timestamp: Date().addingTimeInterval(-172_800))  // 2 days ago
    ...
    let timeRange = DateInterval(
      start: Date().addingTimeInterval(-259_200),  // 3 days ago
      end: Date())

    let adherence = DoseDefaults.calculateAdherence(
      for: profile, in: timeRange, doses: [dose1, dose2, dose3])
  ...
  func doseSummaryComprehensiveStats() throws {
    ...
    let dose1 = Dose(amount: 0.5, timestamp: Date().addingTimeInterval(-1_209_600), site: "Thigh")  // 2 weeks ago
    let dose2 = Dose(amount: 1.0, timestamp: Date().addingTimeInterval(-604_800), site: "Abdomen")  // 1 week ago
    let dose3 = Dose(
      amount: 0.0, timestamp: Date().addingTimeInterval(-604_800 + 3600), skipped: true)  // 1 week ago + 1 hour (skipped)
  ...
  func doseSummaryWithTimeRangeFilter() throws {
    ...
    let dose1 = Dose(amount: 0.5, timestamp: Date().addingTimeInterval(-1_814_400), site: "Thigh")  // 3 weeks ago
    let dose2 = Dose(amount: 1.0, timestamp: Date().addingTimeInterval(-1_209_600), site: "Abdomen")  // 2 weeks ago
    let dose3 = Dose(amount: 1.0, timestamp: Date().addingTimeInterval(-604_800), site: "Thigh")  // 1 week ago
    ...
    let twoWeeksAgo = Date().addingTimeInterval(-1_209_600 - 1)  // 2 weeks ago minus 1 second
    let timeRange = DateInterval(start: twoWeeksAgo, end: Date())

    let summary = DoseDefaults.getDoseSummary(
      for: profile, in: timeRange, doses: [dose1, dose2, dose3, dose4])
  ...
  func defaultStartingDosesByMedication() throws {
    ...
    let semaglutideStartingDose = DoseDefaults.defaultStartingDose(
      for: .semaglutide, brand: "Ozempic")
    #expect(semaglutideStartingDose == 0.25)

    let tirzepatideStartingDose = DoseDefaults.defaultStartingDose(
      for: .tirzepatide, brand: "Mounjaro")
    #expect(tirzepatideStartingDose == 2.5)

    let liraglutideStartingDose = DoseDefaults.defaultStartingDose(
      for: .liraglutide, brand: "Saxenda")
    #expect(liraglutideStartingDose == 0.6)

    let dulaglutideStartingDose = DoseDefaults.defaultStartingDose(
      for: .dulaglutide, brand: "Trulicity")
