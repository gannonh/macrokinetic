...
struct DoseValidationSiteTests {
    ...
    struct InjectionSiteValidationTests {
        ...
        func validInjectionSites() async throws {
        ...
        func invalidInjectionSites() async throws {
            let invalidSites = [
        ...
        func injectionSiteWhitespaceHandling() async throws {
    ...
    struct SiteRotationValidationTests {
        ...
        func validSiteRotation() async throws {
        ...
        func invalidSiteRotation() async throws {
            // Same site as last dose (consecutive use)
            #expect(!DoseValidation.isValidSiteRotation("Thigh", previousSites: ["Thigh"]))
            #expect(
                !DoseValidation.isValidSiteRotation(
                    "Abdomen", previousSites: ["Upper Arm", "Thigh", "Abdomen"]))
        ...
        func siteRotationWithInvalidSites() async throws {
        ...
        func siteRotationWindow() async throws {
            let longHistory = ["Thigh", "Abdomen", "Upper Arm", "Buttocks", "Thigh", "Abdomen"]

            // Custom rotation window - should only check last 2 doses
            #expect(
                DoseValidation.isValidSiteRotation(
                    "Upper Arm", previousSites: longHistory, rotationWindow: 2))
            #expect(
                !DoseValidation.isValidSiteRotation(
                    "Abdomen", previousSites: longHistory, rotationWindow: 2))
