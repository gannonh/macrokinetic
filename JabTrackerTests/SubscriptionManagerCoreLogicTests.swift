...
struct SubscriptionManagerCoreLogicTests {
    ...
    func subscriptionStateTransitionLifecycle() {
        ...
        let premiumStatus = SubscriptionManager.evaluateStatusForTests(from: [trialTx], now: afterTrialDate)
        ...
        let expiredStatus = SubscriptionManager.evaluateStatusForTests(from: [trialTx], now: expiredDate)
    ...
    func stateTransitionEdgeCases() {
        ...
        let beforeStatus = SubscriptionManager.evaluateStatusForTests(from: [txExactExpiry], now: justBeforeExpiry)
        ...
        let boundaryStatus = SubscriptionManager.evaluateStatusForTests(from: [txExactExpiry], now: trialBoundaryDate)
    ...
    func evaluateStatusEdgeCaseMatrix() {
        ...
        #expect(SubscriptionManager.evaluateStatusForTests(from: [recentTrial], now: now) == .trialActive)
        ...
        let oldPremium = SubscriptionManager.EvalInputTest(
            ...
            expirationDate: futureDate)
        #expect(SubscriptionManager.evaluateStatusForTests(from: [oldPremium], now: now) == .premiumActive)
    ...
    func entitlementProcessingScenarios() async {
        ...
        let initialStates: [AppSubscriptionStatus] = [.notSubscribed, .trialActive, .premiumActive, .expired]
