...
enum DoseValidation {
  ...
  static func isValidSiteRotation(
    _ newSite: String, previousSites: [String], rotationWindow: Int = 4
  ) -> Bool {
  ...
  static func isValidDoseTiming(
    _ proposedDate: Date, lastDoseDate: Date?, for medication: Medication
  ) -> Bool {
  ...
  static func validateDose(
    ...
    if !self.isValidDoseTiming(date, lastDoseDate: lastDoseDate, for: medication) {
      errors.append(
        .invalidDoseTiming(proposedDate: date, lastDate: lastDoseDate, medication: medication))
    }
...
enum ValidationError: LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case let .invalidDoseAmount(amount, medication, brand):
      let availableDoses = medication.availableDoses(for: brand).map { String($0) }.joined(
        separator: ", ")
    ...
    case let .invalidDosePrecision(amount, medication):
      return
        "Dose \(amount) mg has invalid precision for \(medication.displayName). Use appropriate increments."
    ...
    case let .invalidDoseTiming(_, lastDate, medication):
      ...
      return
        "Too soon since last dose (\(lastDateDesc)). \(medication.displayName) is \(frequencyDesc)."
    ...
    case let .invalidSiteRotation(site, previousSites):
      ...
      return
        "Avoid using '\(site)' again immediately after '\(lastSite)'. Rotate injection sites for safety."
    }
  }
}
