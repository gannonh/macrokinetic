...
struct WelcomeScreensView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            TabView(selection: $currentWelcomeStep) {
                ForEach(Array(welcomeScreens.enumerated()), id: \.offset) { index, screen in
                    WelcomeScreenContent(screen: screen)
                        .tag(index)
                }
            }
            .tabViewStyle(PageTabViewStyle(indexDisplayMode: .never))
            .animation(.easeInOut, value: currentWelcomeStep)
            
            // Custom page dots
            HStack(spacing: 12) {
                ForEach(0..<welcomeScreens.count, id: \.self) { index in
                    Circle()
                        .fill(index == currentWelcomeStep ? DesignTokens.Colors.primary : Color(.systemGray4))
                        .frame(width: 12, height: 12)
                        .onTapGesture {
                            withAnimation(.spring()) {
                                currentWelcomeStep = index
                            }
                        }
                        .accessibilityLabel("Page \(index + 1) of \(welcomeScreens.count)")
                }
            }
            .padding(.bottom, 24)
    ...
    private func startAutoAdvance() {
        Timer.scheduledTimer(withTimeInterval: 4.0, repeats: true) { timer in
            withAnimation(.spring()) {
                if currentWelcomeStep < welcomeScreens.count - 1 {
