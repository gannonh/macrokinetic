...
/// Legacy onboarding View (v0.5.x and earlier). Retained for reference only.
/// New implementation: JabTracker/Onboarding/OnboardingView.swift
struct LegacyOnboardingView: View {
    @StateObject private var viewModel: LegacyOnboardingViewModel
    ...
    init(isPresented: Binding<Bool>, authManager: AuthenticationManager) {
        ...
        self._viewModel = StateObject(
            wrappedValue: LegacyOnboardingViewModel(
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 0) {
                ...
                .accessibilityLabel(
                    "Step \(self.viewModel.currentStepIndex + 1) of \(self.viewModel.totalSteps)")
