...
/// Legacy onboarding ViewModel (v0.5.x and earlier). Retained for reference only.
/// New implementation: JabTracker/Onboarding/OnboardingViewModel.swift
@MainActor
class LegacyOnboardingViewModel: ObservableObject {
    @Published var currentStep: LegacyOnboardingStep = .welcome {
        didSet {
            self.updateProgress()
        }
    }
    ...
    var totalSteps: Int {
        LegacyOnboardingStep.allCases.count
    }

    var currentStepIndex: Int {
        LegacyOnboardingStep.allCases.firstIndex(of: self.currentStep) ?? 0
    }

    var isLastStep: Bool {
        self.currentStep == LegacyOnboardingStep.allCases.last
    }
    ...
    func moveToNextStep() {
        ...
        if let currentIndex = LegacyOnboardingStep.allCases.firstIndex(of: currentStep),
            currentIndex + 1 < LegacyOnboardingStep.allCases.count
        {
            self.currentStep = LegacyOnboardingStep.allCases[currentIndex + 1]
        }
    }
