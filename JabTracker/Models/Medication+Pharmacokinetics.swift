...
extension Medication {
    ...
    func concentrationAtTime(
        ...
        return initialConcentration * exp(-eliminationRateConstant * timeElapsedHours)
    }
    ...
    func fractionRemainingAfter(timeElapsedHours: Double) -> Double {
        ...
        return exp(-eliminationRateConstant * timeElapsedHours)
    }
    ...
    func steadyStateProgress(timeOnMedicationHours: Double) -> Double {
        ...
        let progress = min(timeOnMedicationHours / timeToSteadyStateHours, 1.0)
...
extension Medication {
    ...
    var hasValidPharmacokinetics: Bool {
        halfLifeDays > 0 &&
               halfLifeDays < 30 &&  // No medication should have >30 day half-life
               subcutaneousBioavailability > 0 &&
               subcutaneousBioavailability <= 1.0 &&
               peakTimeHours > 0 &&
               peakTimeHours < 168  // No peak time should exceed 1 week
    }

    /// Returns a validation error message if pharmacokinetics are invalid
    var pharmacokineticValidationError: String? {
        if !hasValidPharmacokinetics {
