...
extension MedicationProfile {
    ...
    var timeBasedEffectivenessInsights: [EffectivenessInsight] {
        ...
        guard let med = medication else {
            return [EffectivenessInsight(period: "Unknown", effectivenessChange: nil, note: "Medication type not set")]
        }

        let daysOnMedication = Calendar.current.dateComponents([.day], from: self.startDate, to: Date()).day ?? 0
