...
extension Dose {
    ...
    var daysSinceLastDose: Int? {
        ...
        let relevantDoses = userDoses.filter { dose in
            dose.medication?.id == medication.id &&
                dose.timestamp < self.timestamp &&
                !dose.skipped
        }
        ...
        let daysBetween = calendar.dateComponents([.day], from: lastDose.timestamp, to: self.timestamp).day
        return daysBetween
    }
    ...
    var isFirstDoseOfWeek: Bool {
        ...
        let sameMedicationThisWeek = userDoses.filter { dose in
            dose.medication?.id == medication.id &&
                dose.timestamp >= weekInterval.start &&
                dose.timestamp < self.timestamp &&
                !dose.skipped
        }
    ...
    var currentStreak: Int {
        ...
        for doseIndex in currentDoseIndex ..< relevantDoses.count {
            ...
            if doseIndex + 1 < relevantDoses.count {
                ...
                let daysBetween = calendar.dateComponents([.day], from: nextDose.timestamp, to: dose.timestamp).day ?? 0
            ...
            if dose.skipped {
                ...
                if doseIndex + 1 < relevantDoses.count {
                    ...
                    let daysBetween = calendar.dateComponents(
                        [.day], from: nextDose.timestamp, to: dose.timestamp).day ?? 0
