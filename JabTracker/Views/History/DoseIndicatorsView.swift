...
#Preview("Various States") {
    VStack(spacing: 16) {
        ...
        // Mixed: 2 logged + 1 scheduled
        HStack {
            Text("2 Logged + 1 Scheduled:")
            DoseIndicatorsView(events: [
                DoseEvent(
                    id: UUID(),
                    timestamp: Date(),
        ...
        // Overflow: 5 total (3 logged + 2 scheduled)
        HStack {
            Text("5 Total (+2 overflow):")
            DoseIndicatorsView(events: [
                DoseEvent(
                    id: UUID(),
                    timestamp: Date(),
