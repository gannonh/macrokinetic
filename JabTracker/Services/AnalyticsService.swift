...
final class AnalyticsService {
  ...
  func calculateUserAnalytics(user: User, context: ModelContext) -> UserAnalyticsSummary {
    ...
    let insights = self.generateAdherenceInsights(
      user: user, effectiveness: medicationEffectiveness)
  ...
  func calculateMedicationEffectiveness(
    ...
    let thirtyDaysAgo = Calendar.current.date(byAdding: .day, value: -30, to: Date()) ?? Date()
    let adherenceFactor = profile.calculateAdherence(
      from: thirtyDaysAgo, to: Date(), context: context)

    // Concentration optimality (based on therapeutic range)
    let concentrationOptimality = self.calculateConcentrationOptimality(
  ...
  func generateConcentrationInsights(user: User, context: ModelContext) -> [ConcentrationInsight] {
    (user.medicationProfiles ?? []).compactMap { profile in
      ...
      let thirtyDaysAgo = Calendar.current.date(byAdding: .day, value: -30, to: Date()) ?? Date()
      let adherenceRate = profile.calculateAdherence(
        from: thirtyDaysAgo, to: Date(), context: context)
      let daysOnMedication =
        Calendar.current.dateComponents([.day], from: profile.startDate, to: Date()).day ?? 0
  ...
  private func calculateConcentrationOptimality(profile: MedicationProfile, user: User) -> Double {
    ...
    if currentConcentration >= therapeuticWindow.lowerBound,
      currentConcentration <= therapeuticWindow.upperBound
    {
      // Within range - calculate optimality based on position within range
      let rangePosition =
        (currentConcentration - therapeuticWindow.lowerBound)
      ...
      let excessRatio =
        (currentConcentration - therapeuticWindow.upperBound) / therapeuticWindow.upperBound
  ...
  private func analyzeConcentrationTrend(profile: MedicationProfile) -> ConcentrationTrend {
    ...
    let peakToTroughVariability =
      maxConcentration > 0 ? (maxConcentration - minConcentration) / maxConcentration : 0.0
  ...
  private func generateAdherenceInsights(
    ...
    let overallAdherence =
      effectiveness.map(\.adherenceRate).reduce(0.0, +) / Double(effectiveness.count)
    ...
    for medicationEffectiveness in effectiveness {
      if medicationEffectiveness.effectivenessScore < 0.6,
        medicationEffectiveness.adherenceRate > 0.8
      {
        insights.append(
          AdherenceInsight(
  ...
  private func generateRecommendations(
    ...
    if concentration < 0.6 {
      recommendations.append(
        "Current concentration below optimal range - consider dose timing optimization")
