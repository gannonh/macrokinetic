...
**Content Definition:**
- If flagged in §4.1, first add an ad-hoc delegation step: "Ad-Hoc Delegation – <purpose>" (optional ref to .windsurf/workflows/apm-7-delegate-research.md or .windsurf/workflows/apm-8-delegate-debug.md), then continue
...
2. **If User skips Review:** Proceed directly to **Manager Bootstrap Creation**.
  - **CRITICAL:** You must generate the Bootstrap Prompt using the **EXACT TEMPLATE** defined in your initiation prompt .windsurf/workflows/apm-1-initiate-setup.md.
  - **Context Recovery:** If you cannot retrieve the template word-for-word from your context, you must **READ** the .windsurf/workflows/apm-1-initiate-setup.md file to refresh your memory before generating the prompt. Do not approximate the template.
