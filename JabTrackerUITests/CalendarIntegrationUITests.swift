...
final class CalendarIntegrationUITests: XCTestCase {
    ...
    func test_calendar_monthNavigation() throws {
        ...
        if monthYearHeader.exists {
            ...
            let nextMonthTexts = app.staticTexts.matching(NSPredicate(format: "label CONTAINS %@", expectedNextMonthYear))
        ...
        if !prevMonthButton.exists {
            ...
            let prevNavButtons = app.buttons.matching(NSPredicate(format: "label CONTAINS '<' OR label CONTAINS 'previous'"))
        ...
        if monthYearHeader.exists {
            ...
            let currentMonthTexts = app.staticTexts.matching(NSPredicate(format: "label CONTAINS %@", currentMonthYear))
    ...
    func test_calendar_emptyMonthHandling() throws {
        ...
        if monthYearHeader.exists {
            ...
            XCTAssertGreaterThan(monthTextsBack.count, 0, "Month header should still be visible after navigation (fallback check)")
        }
    }
