...
final class CloudKitIntegrationUITests: XCTestCase {
    ...
    func testCloudKitRetryFunctionality() throws {
        ...
        if app.buttons["retry-sync-button"].exists {
            ...
            let statusUpdated =
                app.staticTexts["Checking sync status..."].waitForExistence(timeout: 3)
    ...
    func testSyncStatusReflectsActualCloudKitState() throws {
        ...
        let cloudKitStatusTexts = allStatusTexts.filter { text in
            text.contains("iCloud") || text.contains("Sync") || text.contains("network")
                || text.contains("restricted") || text.contains("Checking")
        }

        XCTAssertGreaterThan(
            cloudKitStatusTexts.count, 0,
        ...
        let hasRealisticStatus = cloudKitStatusTexts.contains { status in
            ...
            status.contains("Syncing with iCloud") || status.contains("Sign in to iCloud")
                || status.contains("Sync unavailable") || status.contains("network connection")
...
extension CloudKitIntegrationUITests {
    ...
    private func waitForCloudKitStatusStabilization(
        _ app: XCUIApplication, timeout: TimeInterval = 15
    ) {
        let startTime = Date()
        var lastStatus = ""
        var stableCount = 0
